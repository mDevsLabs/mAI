---
title: Glossary
description: Definitions of the core concepts and terms used across mAI, from agents and topics to skills, memory, models, and billing.
tags:
  - mAI
  - Glossary
  - Terminology
...
A quick reference for the words you'll meet across mAI. Terms are grouped by their first letter; common synonyms point to the main entry.
...
Anthropic's terminal coding agent that you can drive from the mAI desktop app. mAI runs the local `claude` CLI and renders its tasks, todos, skills, and tool calls as chat blocks.
...
OpenAI's terminal coding agent that you can drive from the mAI desktop app. mAI runs the local `codex` CLI and renders its file changes, todos, and command output as chat blocks.
...
The unit mAI uses to measure AI usage. Different models consume different amounts per request, and credits are drawn from your free allowance, subscription, referral rewards, and top-ups.
...
A numeric (vector) representation of text that lets content be matched by meaning. mAI embeds your files so they can be found through semantic search.
...
mAI's built-in model provider that gives you access to mainstream models without configuring your own API keys, billed with credits.

## M

### MCP (Model Context Protocol)

An open standard for connecting agents to external tools, data, and services. mAI acts as the MCP client; MCP servers run locally (STDIO) or remotely (Streamable HTTP) and expose tools, resources, and prompts.

### Memory

Personal context (preferences, role, work habits, and experiences) that agents extract from conversations and recall later. mAI keeps memory white-box: you can view, edit, delete, and add entries, and agents remember only what you approve.

### Messenger

A no-setup way to chat with your own mAI agents through the official `@mAI` bot on Telegram, Slack, and Discord. Unlike Channels, only you talk to the bot.
...
A service that supplies models, such as OpenAI, Anthropic, Google, or local options like Ollama. mAI integrates many providers, configured under Settings. Also called a Provider.
...
A long-form document (文稿) you write and edit in mAI, with Markdown, live preview, version history, and auto-save. Agents can draft and revise pages directly through the Pages agent.
...
Running open-source mAI on your own infrastructure (Docker, Vercel, or other platforms) for full control of your data. Commercial features such as credits and subscriptions are inactive in self-hosted deployments.

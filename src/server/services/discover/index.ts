    ...
    try {
      ...
      const assistant = {
        ...
        homepage:
          ...
          `https://mprojects.odoo.com/discover/assistant/${(data as any).identifier}`,
    ...
    try {
      ...
      const transformedItems: DiscoverAssistantItem[] = (data.items || []).map((item: any) => {
        ...
        return {
          ...
          homepage: item.homepage || `https://mprojects.odoo.com/discover/assistant/${item.identifier}`,
          identifier: item.identifier,
    ...
    try {
      ...
      const items: DiscoverAssistantItem[] = (data.items || []).map((item: any) => {
        ...
        return {
          ...
          homepage: `https://mprojects.odoo.com/discover/assistant/${item.identifier}`,
          identifier: item.identifier,
          installCount: item.installCount,
  ...
  getPluginDetail = async (params: {
    ...
    if (builtinTool) {
      ...
      const plugin: DiscoverPluginDetail = {
        ...
        homepage: 'https://mprojects.odoo.com',
    ...
    if (klavisTool) {
      ...
      const plugin: DiscoverPluginDetail = {
        ...
        category: undefined,
        createdAt: '',
        description: `mAI Mcp Server: ${klavisTool.label}`,
  ...
  getUserInfo = async (params: {
    ...
    try {
      ...
      const transformedAgents: DiscoverAssistantItem[] = (agents || []).map((agent: any) => ({
        ...
        homepage: `https://mprojects.odoo.com/discover/assistant/${agent.identifier}`,
      ...
      const transformedPlugins: DiscoverPluginItem[] = (plugins || []).map((plugin: any) => ({
        ...
        homepage: `https://mprojects.odoo.com/discover/plugin/${plugin.identifier}`,

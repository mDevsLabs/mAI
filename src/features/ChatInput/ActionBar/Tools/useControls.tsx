...
const officialTag = (
  <Tooltip placement={'top'} title={'mAI'}>
...
export const useControls = ({ closeDropdown }: { closeDropdown?: () => void } = {}) => {
  ...
  // Load user's mAI Skill connections via SWR
        ...
        ? COMPOSIO_APP_TYPES.filter(
            ...
            if (server?.status === ComposioServerStatus.ACTIVE) {
              return createManagedSkillItem({
                ...
                extraTag: type.author === 'mAI' ? officialTag : undefined,
                icon,
  ...
  // mAI Skill Provider list items - only show installed or recommended
      ...
      isLobehubSkillEnabled
        ? LOBEHUB_SKILL_PROVIDERS.filter(
            ...
            if (server?.status === LobehubSkillStatus.CONNECTED || server?.isConnected) {
              return createManagedSkillItem({
                badge: <Icon icon={McpIcon} size={12} />,
                extraTag: provider.author === 'mAI' ? officialTag : undefined,
                icon,
                id: server.identifier,
                popoverContent,
  ...
  // Builtin tool list items (excluding Composio and mAI Skill)
  ...
  // Builtin Agent Skills list items (grouped under mAI)
  ...
  // Skills list items (including mAI Skill and Composio)
  // Connected items listed first, deduplicated by key (mAI takes priority)
  const skillItems = useMemo(() => {
    // Deduplicate by key - mAI items take priority over Composio
    const seenKeys = new Set<string>();
    const allItems: typeof lobehubSkillItems = [];

    // Add mAI items first (they take priority)
  ...
  // integrations (Composio/mAI Skill gateway plugins with other sources like
  // 'self'/'builtin') don't leak in here and duplicate the brand-icon items
  // already rendered under the mAI group.
  ...
  // Build mAI group children (including Builtin Agent Skills, builtin tools, and mAI Skill/Composio)
  const lobehubGroupChildren: ItemType[] = [
    // 1. Builtin Agent Skills
    ...builtinAgentSkillItems,
    // 2. Builtin tools
    ...builtinItems,
    // 3. mAI Skill and Composio (as builtin skills)
    ...skillItems,
  ];
  ...
  // (e.g. a Composio/mAI integration item plus an installed plugin sharing
  // the same identifier), which would otherwise render the row twice. Keep the
  // first occurrence so the richer integration item (mAI group, listed
  ...
  const installedPluginItems: ItemType[] = useMemo(() => {
    ...
    // Connected mAI Skill Providers
    const connectedLobehubSkillItems = lobehubSkillItems.filter((item) =>
      checked.includes(item.key as string),
    );

    // Merge enabled mAI Skill and Composio (as builtin skills)
    ...
    // Build builtin tools group children (including Builtin Agent Skills, builtin tools, and mAI Skill/Composio)

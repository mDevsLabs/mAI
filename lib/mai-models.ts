...
export function isMaiModel(modelId: string): boolean {
  const normalized = (modelId || "").toLowerCase().trim();
  return maiModelsList.some(
    (m) =>
      m.id.toLowerCase() === normalized ||
      m.name.toLowerCase() === normalized ||
      m.ollamaTag?.toLowerCase() === normalized ||
      m.huggingFaceTag?.toLowerCase() === normalized ||
      m.apiAlias?.toLowerCase() === normalized
  );
}

...
export const patchTextNodes = (schema, oldNode, newNode) => {
  ...
  // Map diffs to nodes
  const res = diffs.flatMap(([type, sentences]) => sentences.map((sentence) => {
...
export const createDiffNode = (schema, node, type) => mapDocumentNode(node, (currentNode) => {
...
export const createTextNode = (schema, content, marks = []) => schema.text(content, marks);

...
import type { mAIDatabase } from '@lobechat/database';
...
export class AiAgentService {
  ...
  async execAgent(params: InternalExecAgentParams): Promise<ExecAgentResult> {
    ...
    if (isHeteroAgent) {
      ...
      // standard 'github' key (mAI OAuth connector default); agent config can
    ...
    if (params.disableTools) {
      ...
      const pluginIds = [
        ...new Set([
          ...
          // Include mAI Skills and Composio tools so they are passed to generateToolsDetailed
          ...lobehubSkillManifests.map((m) => m.identifier),
          ...composioManifests.map((m) => m.identifier),
      ...
      // installed plugin, a mAI Skill, or a Composio manifest declaring

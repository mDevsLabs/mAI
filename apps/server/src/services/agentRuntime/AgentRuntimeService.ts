...
export class AgentRuntimeService {
  ...
  constructor(db: mAIDatabase, userId: string, options?: AgentRuntimeServiceOptions) {
  ...
  async executeStep(params: AgentExecutionParams): Promise<AgentExecutionResult> {
    ...
    try {
      ...
      // `_partial/<op>.json.br` and the canonical
      // `agent-traces/<agentId>/<topicId>/<op>.json.br` returns 404 — see
      // .
      //

 ...
 * which mAI context a conversation runs in is derived from the *active
...
export const messengerRouter = router({
   ...
   * a mAI account — the page uses it to warn the user before they create
  ...
  peekLinkToken: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return {
        // Set when the IM identity is already linked to some mAI account.
  ...
  confirmLink: messengerProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // already bound to a different mAI account. The DB unique index
      ...
      try {
        ...
        // Race backstop: the IM identity got bound to another mAI user
  ...
  /**
   * List the Slack workspaces this mAI user has installed the bot into.
  ...
  uninstallInstallation: messengerWriteProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // it. Workspace admins who installed via a different mAI account

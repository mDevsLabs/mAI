...
describe('getServerGlobalConfig', () => {
  ...
  it('should only enable mAI by default in business feature mode', async () => {
    const providerConfig = await loadCapturedProviderConfig(true);

    expect(providerConfig[ModelProvider.mAI].enabled).toBe(true);
    ...
    for (const provider of Object.values(ModelProvider)) {
      if (provider === ModelProvider.mAI) continue;
  ...
  it('should keep upstream defaults outside business feature mode', async () => {
    ...
    expect(providerConfig[ModelProvider.mAI]).toBeUndefined();

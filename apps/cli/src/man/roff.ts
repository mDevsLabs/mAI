...
const EXAMPLES = [
  ...
  {
    command: 'mai connect --daemon',
    description: 'Start the device gateway connection in the background.',
  },
  {
    command: 'mai search -q "gpt-5"',
    description: 'Search local resources for a query.',
  },
  {
    command: 'mai generate text "Write release notes"',
    description: 'Generate text from a prompt.',
  },
  {
    command: 'mai man generate',
...
export function generateRootManPage(program: Command, version: string) {
  ...
  const lines = [
    ...
    `.TH MAI 1 "" "${escapeRoff(`@mdevs/mai-cli ${version}`)}" "User Commands"`,
    '.SH NAME',
    `mai \\- ${escapeRoff(program.description() || 'mAI CLI')}`,
    '.SH SYNOPSIS',
    ...formatSynopsisLines(),
    '.SH DESCRIPTION',
    escapeRoff(
      `${program.name()} is the command-line interface for mAI. It provides authentication, device gateway connectivity, content generation, resource search, and management commands for agents, files, models, providers, plugins, knowledge bases, threads, topics, and related resources.`,
    ),
    '.PP',

...
export class TaskModel {
  ...
  constructor(db: mAIDatabase, userId: string, workspaceId?: string) {
    this.db = db;
    this.userId = userId;
  ...
  static async getScheduledTasks(db: mAIDatabase): Promise<TaskItem[]> {
    return db
      .select()
      .from(tasks)
      .where(
  ...
  static async findStuckTasks(db: mAIDatabase): Promise<TaskItem[]> {
    return db
      .select()
      .from(tasks)
      .where(
   ...
   */
  private async collectTaskSubtree(rootId: string, runner: mAIDatabase): Promise<TaskItem[]> {
  ...
  async transferTo(
    ...
    return this.db.transaction(async (trx) => {
      const scoped = new TaskModel(trx as mAIDatabase, this.userId, this.workspaceId);
      const subtree = await scoped.collectTaskSubtree(taskId, trx as mAIDatabase);
  ...
  async copyToWorkspace(
    ...
    return this.db.transaction(async (trx) => {
      ...
      let seq = await this.nextSeqIn(trx as mAIDatabase, targetWorkspaceId, targetUserId);
      ...
      while (queue.length > 0) {
        ...
        const identifier = `T-${seq}`;
        const inserted = (await (trx as mAIDatabase)
          .insert(tasks)
          .values({

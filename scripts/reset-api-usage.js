      ...
      <div style="max-width:560px; margin:40px auto; background:#111827; border:1px solid #1f293d; border-radius:16px; overflow:hidden; box-shadow:0 20px 25px -5px rgba(0,0,0,0.5);">
        ...
        <div style="padding:32px 28px; line-height:1.6; font-size:15px; color:#cbd5e1;">
          ...
          <div style="background:#1e293b; border:1px solid #334155; border-radius:12px; padding:20px; text-align:center; margin:24px 0;">
            ...
            <p style="font-size:13px; color:#94a3b8; margin:12px 0 0 0;">Vous pouvez à nouveau utiliser l'ensemble de vos crédits et clés API mProjects.</p>
          </div>
        </div>
...
async function main() {
  ...
  for (const user of users) {
    const html = buildEmailHtml("Réinitialisation des quotas API", user.username, "Vos quotas API mProjects ont été réinitialisés pour le mois.");
    ...
    if (gmailAppPass) {
      try {
        ...
        await transporter.sendMail({
          ...
          to: user.email,
          subject: "Vos quotas API mProjects ont été réinitialisés",
          html,
        });
        success++;

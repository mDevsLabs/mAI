...
// Import nommé DataGrid suite à la mise à jour de react-data-grid (v7-beta.59) qui n'a plus de default export
...
const PureSpreadsheetEditor = ({ content, saveContent }: SheetEditorProps) => {
  ...
  const columns = useMemo(() => {
    ...
    const dataColumns = Array.from({ length: MIN_COLS }, (_, i) => ({
      ...
      // Désactivé suite à la mise à jour de react-data-grid (v7-beta.59) qui pose problème avec textEditor
  ...
  const initialRows = useMemo(() => parseData.map((row, rowIndex) => {
  ...
  const handleRowsChange = (newRows: Record<string, string | number>[]) => {
    ...
    const updatedData = newRows.map((row) => columns.slice(1).map((col) => String(row[col.key] ?? "")));

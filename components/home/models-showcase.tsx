...
export function ModelsShowcase() {
  ...
  // Série mise en avant : la nouvelle génération cloud mAI-2
      ...
      >
        <div>
          ...
          <p className="text-slate-500 text-sm sm:text-base font-light mt-1 max-w-xl">
            La nouvelle génération de mAI, exécutée dans le cloud via l&apos;API mAI : contexte jusqu&apos;à 1 million de tokens, texte + images, pour tous les forfaits.
          </p>
        </div>

        <Link
          href="/models"
          className="group inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/60 backdrop-blur-md border border-slate-200 hover:border-sky-300 text-slate-900 text-sm font-bold transition-all shadow-sm w-fit"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {flagshipModels.map((model, index) => (
          <motion.div
            ...
            className="group relative bg-white/80 backdrop-blur-md border border-slate-200/80 rounded-3xl p-6 hover:border-sky-400/60 transition-all duration-300 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-xl flex flex-col justify-between overflow-hidden"
          >
            <div>
              {/* Header card avec l'image ou icône */}
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-sky-500/10 text-sky-700 border border-sky-500/20">
                  <Cloud className="w-3 h-3" /> Cloud · API mAI
                </span>
              </div>

              {/* Titre & Badges */}
              <h3 className="text-2xl font-black text-slate-900 mb-2 group-hover:text-sky-600 transition-colors">
                {model.name}
              </h3>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-200/60">
              {/* Commande d'appel : Ollama en local, alias API en cloud */}
              <div className="flex items-center justify-between bg-slate-900 text-slate-200 p-2.5 rounded-2xl text-xs font-mono">
                <div className="flex items-center gap-2 truncate pr-2">
                  ...
                  <span className="truncate text-slate-300 select-all">
                    {model.cloud ? `model: "${model.apiAlias}"` : `ollama run ${model.ollamaTag}`}
                  </span>
                </div>
                <motion.button
                  whileTap={{ scale: 0.9 }}
                  onClick={() => handleCopy(model.cloud ? (model.apiAlias || model.id) : (model.ollamaTag || model.id))}
                  className="p-1.5 rounded-xl hover:bg-slate-800 text-slate-300 hover:text-white transition-colors shrink-0"
                  title={model.cloud ? "Copier l'identifiant API" : "Copier la commande Ollama"}
                >
                  {(model.cloud ? (model.apiAlias || model.id) : (model.ollamaTag || model.id)) === copiedTag ? (
              ...
              <div className="flex items-center gap-2">
                <Link
                  ...
                  className="w-full text-center py-2.5 rounded-xl bg-slate-900 text-white font-bold text-xs hover:bg-sky-600 transition-colors"
                >
                  Fiche Technique
                </Link>
              </div>

      ...
      >
        <div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black italic tracking-tighter uppercase text-white">
            La Suite <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-blue-400 to-emerald-300">mAI</span>
          </h2>
        ...
        <Link
          href="/projects"
          className="group inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/15 hover:border-blue-300/50 text-white text-sm font-bold transition-all shadow-lg w-fit"
        >
          Tous les projets
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {projects.map((project, index) => (
          <motion.div
            ...
            className={`group relative bg-white/[0.06] backdrop-blur-md border border-white/10 rounded-3xl p-5 sm:p-7 transition-colors duration-200 shadow-[0_4px_30px_rgba(0,0,0,0.35)] flex flex-col justify-between overflow-hidden ${project.highlightColor}`}
          >
            <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity pointer-events-none">
              <span className="text-6xl sm:text-7xl font-black italic tracking-tighter select-none text-white">
                0{index + 1}
              </span>
            </div>
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/10 border border-white/15 text-white shadow-md p-2 group-hover:scale-105 transition-transform duration-200">
              ...
              <p className="text-xs font-semibold text-slate-400 italic mb-2.5">
                &quot;{project.tagline}&quot;
              </p>
              ...
              <div className="flex flex-wrap gap-1.5 mb-5">
                {project.platforms.map((plat) => (
                  <span
                    key={plat}
                    className="text-[10px] px-2.5 py-0.5 rounded-full bg-white/10 border border-white/15 text-slate-200 font-bold uppercase tracking-wider"
                  >
                    {plat}
                  </span>
                ))}
            ...
            <div className="relative z-10 pt-3.5 border-t border-white/10 flex items-center justify-between gap-3 flex-wrap">
              <Link
                href={project.link}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-purple-600 text-white text-xs font-bold hover:bg-purple-500 transition-colors shadow-md"
              >
